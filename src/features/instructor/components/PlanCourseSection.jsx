    ...
    <div className="mb-8">
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          {minItems > 0 && <p className="text-sm text-gray-500 mt-1">At least {minItems}</p>}
        </div>
        {isEditable && (
          <button onClick={() => addItem(field, { text: '' })} className="flex items-center gap-1 px-3 py-1.5 text-sm text-purple-600 hover:bg-purple-50 rounded-lg">
            <HiPlus size={14} /> Add
          </button>
        )}
      </div>
      <div className="space-y-3">
        {(data[field] || []).map((item) => (
          <div key={item.id} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg group">
            ...
            />
            {isEditable && (
              <button onClick={() => deleteItem(field, item.id)} className="opacity-0 group-hover:opacity-100 transition p-1 text-red-500 hover:text-red-700">
                <HiTrash size={16} />
              </button>
            )}
          </div>
        ...
        {isOpen && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-gray-100">
            <div className="p-6 space-y-6">
              {renderTextList('learningObjectives', 'What will students learn?', 'e.g., "Build full‑stack apps with React"', 4, <HiCheckCircle size={18} className="text-green-500" />)}
