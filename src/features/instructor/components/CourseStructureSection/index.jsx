...
const CourseStructureSection = ({ data, onUpdate, isEditable }) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <SectionHeader icon={HiBookOpen} title="Course Structure" isOpen={isOpen} onToggle={() => setIsOpen(!isOpen)} />
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-gray-100">
            <div className="p-6">
              <div className="mb-6 p-4 bg-purple-50 rounded-xl flex gap-6">
                <div><p className="text-xs text-gray-500">Sections</p><p className="text-2xl font-bold text-purple-600">{sections.length}</p></div>
                <div><p className="text-xs text-gray-500">Lessons</p><p className="text-2xl font-bold text-purple-600">{totalLessons}</p></div>
                <div><p className="text-xs text-gray-500">Quizzes</p><p className="text-2xl font-bold text-purple-600">{totalQuizzes}</p></div>
                <div><p className="text-xs text-gray-500">Total Duration</p><p className="text-2xl font-bold text-purple-600">{formatDuration(totalDuration)}</p></div>
              </div>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-4">
                  <div className="flex gap-3">
                    <input
                      ...
                      className="flex-1 px-4 py-2 rounded-lg border border-gray-300 focus:border-purple-400 outline-none"
                    />
                    <button
                      onClick={addSection}
