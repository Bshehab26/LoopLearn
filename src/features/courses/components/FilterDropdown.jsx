 ...
 * NOW SUPPORTS SINGLE CATEGORY SELECTION ONLY (radio buttons instead of checkboxes)
 */
...
/**
 * Calculates course count per category from the courses list
 */
...
/**
 * Extracts category filter from URL parameters (single category)
 */
...
/**
 * Builds URL query string from selected category
 */
const buildQueryString = (selectedCategory) => {
...
const FilterButton = ({ isOpen, onClick, hasActiveFilter, disabled }) => (
  <motion.button
    ...
    style={{
      borderColor: hasActiveFilter ? '#534AB7' : undefined,
      color: hasActiveFilter ? '#534AB7' : '#5F5E5A',
    }}
  >
    <HiOutlineAdjustmentsHorizontal 
      size={16} 
      className={hasActiveFilter ? 'text-purple-600' : ''} 
    />
    Filter
    {hasActiveFilter && (
      <span className='text-white text-xs rounded-full px-1.5 py-0.5' style={{ background: '#534AB7' }}>
        1
      </span>
    )}
    <HiChevronDown 
...
/**
 * Category list with RADIO BUTTONS (single selection)
 */
        ...
        {categories.map(({ label, count }) => (
          ...
          >
            ...
            <div
              ...
              style={{
                borderColor: selectedCategory === label ? '#534AB7' : '#D1D5DB',
              }}
            >
            ...
            {/* Label */}
            <span className='text-sm flex-1 text-gray-700 group-hover:text-purple-600 transition'>
              {label}
            </span>
...
const ApplyButton = ({ onClick, hasSelection, loading }) => (
  <div className='sticky bottom-0 bg-white px-5 py-4 border-t border-gray-100'>
    <motion.button
      ...
      className='w-full py-2.5 rounded-xl text-sm font-semibold text-white transition-all bg-gradient-to-r from-purple-600 to-purple-800 hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed'
    >
      {loading ? (
...
const FilterDropdown = ({ courses = [], onFilterChange }) => {
  ...
  const categoriesWithCounts = useMemo(() => 
    calculateCategoryCounts(courses), 
    [courses]
  );
  ...
  // Parse URL param on mount and route changes
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const categoryFromUrl = extractCategoryFromUrl(params);
    console.log('[FilterDropdown] Category from URL:', categoryFromUrl);
  ...
  useEffect(() => {
    if (isOpen && dropdownRef.current) {
      ...
      if (rect.right + DROPDOWN_WIDTH > screenWidth) {
  ...
  const clearFilter = useCallback(() => {
    console.log('[FilterDropdown] Clear filter clicked');
  ...
  // Apply filter (single category)
  const applyFilter = useCallback(() => {
    if (!selectedCategory) return;
    
    console.log('[FilterDropdown] Applying filter for category:', selectedCategory);
    
    // Update URL with selected category
    const queryString = buildQueryString(selectedCategory);
    const newUrl = queryString ? `/course-list?${queryString}` : '/course-list';
  ...
  useEffect(() => {
    if (courses.length > 0) {
      ...
      console.log('[FilterDropdown] Available categories:', uniqueCategories);
    }
  }, [courses]);
    ...
    <div className='relative' ref={dropdownRef}>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            {...DROPDOWN_ANIMATION}
            className={`absolute ${dropdownPosition} mt-2 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 overflow-hidden`}
