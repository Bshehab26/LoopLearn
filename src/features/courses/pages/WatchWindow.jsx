...
import {getYouTubeId , formatDuration , parseDurationToSeconds} from '../../../shared/utils/helpers';
...
const WatchWindow = () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (resumeItem) {
          ...
          const sectionIndex = data.sections.findIndex((s) => s.id === resumeItem.sectionId);
          if (sectionIndex !== -1) {
          ...
          if (firstSection) {
            ...
            if (firstLesson) {
              setCurrentItem({ type: 'lesson', data: firstLesson, sectionId: firstSection.id });
            } else if (firstQuiz) {
              setCurrentItem({ type: 'quiz', data: firstQuiz, sectionId: firstSection.id });
            }
          }
        }
  ...
  const selectItem = useCallback((item) => {
    ...
    if (section) {
      const type = section.lessons.some((l) => l.id === item.id) ? 'lesson' : 'quiz';
      const data =
        type === 'lesson'
  ...
  const goToNextItem = useCallback(() => {
    ...
    const currentIndex = combined.findIndex(
      (item) => item.id === currentItem.data.id && item.type === currentItem.type
    );
    ...
    if (next) {
      ...
      if (itemData) {
        setCurrentItem({ type: next.type, data: itemData, sectionId: currentSection.id });
        setViewMode('content');
      }
    } else {
      // Next section
      ...
      if (currentSectionIndex < sections.length - 1) {
        ...
        if (firstItem) {
          const type = nextSection.lessons.some((l) => l.id === firstItem.id) ? 'lesson' : 'quiz';
          const data =
            type === 'lesson'
          ...
          setOpenSections((prev) => ({ ...prev, [currentSectionIndex + 1]: true }));
          setViewMode('content');
        }
      }
    }
  ...
  const markLessonComplete = useCallback(async () => {
    ...
    try {
      ...
      // Sync with server to get latest progress and course completion status
  ...
  }, [currentItem, refreshProgress, goToNextItem]);
  ...
  const renderContent = useCallback(() => {
    ...
    return null;
  }, [viewMode, currentItem, completedItems, course, markLessonComplete, handleQuizComplete, goToNextItem]);

  // ── Loading / Error states ──────────────────────────────────────────────
  if (loading) return <WatchWindowSkeleton />;
  if (error || !course) return <EmptyState onBrowseCourses={() => navigate('/courses')} />;
